...
std::vector<std::string> CreateCasFlags(std::string downloader_path,
  ...
  std::set<std::string> autopopulatedFlags = {
      kFlagCasInstance, kFlagCasAddr,      kFlagDigest,
      kFlagDir,         kFlagDisableCache, kFlagServiceAccountJson,
      kFlagUseAdc};
  auto is_autopopulated = [&autopopulatedFlags](std::string flag) {
    return autopopulatedFlags.find(flag) != autopopulatedFlags.end();
  };
  ...
  auto is_timeout_option = [](std::string flag) {
    return (flag.find("_timeout") == sizeof(flag) - sizeof("timeout"));
  };
  ...
  for (const auto& [flag, value] : rpc_options) {
    if (is_supported(flag)) {
      std::string flag_str =
          "-" + flag + "=" + value + (is_timeout_option(flag) ? "s" : "");
      cas_flags.push_back(flag_str);
    }
  }
...
Command GetCommand(const std::string downloader_path,
  ...
  cmd.AddParameter("-" + std::string(kFlagDigest) + "=" +
                   cas_identifier.digest);
