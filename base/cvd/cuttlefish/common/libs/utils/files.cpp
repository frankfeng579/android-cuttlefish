...
Result<std::vector<std::string>> HardLinkDirecoryContentsRecursively(
  ...
  const std::function<bool(const std::string&)> linker =
      [&files, &source, &destination](const std::string& filepath) mutable {
...
Result<std::vector<std::string>> MoveDirectoryContents(
  ...
  const std::function<bool(const std::string&)> file_collector =
      [&files, &destination, &source](const std::string& filepath) mutable {
  ...
  CF_EXPECT(WalkDirectory(source, file_collector));
