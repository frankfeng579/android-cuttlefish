...
TEST_F(FilesTests, HardLinkRecursivelyFailsIfSourceIsNotADirectory) {
  Result<std::vector<std::string>> result = HardLinkDirecoryContentsRecursively(
...
TEST_F(FilesTests, HardLinkRecursively) {
  Result<std::vector<std::string>> result =
      HardLinkDirecoryContentsRecursively(src_dir_, dst_dir_);

  EXPECT_THAT(result, IsOk());
  EXPECT_THAT(result.value(),
              UnorderedElementsAre(dst_dir_ + "/file1.txt",
...
TEST_F(FilesTests, MoveDirectoryContents) {
  Result<std::vector<std::string>> result =
      MoveDirectoryContents(src_dir_, dst_dir_);
  EXPECT_THAT(result, IsOk());
  EXPECT_THAT(result.value(),
              UnorderedElementsAre(dst_dir_ + "/file1.txt",
                                   dst_dir_ + "/sub_dir/file2.txt"));
